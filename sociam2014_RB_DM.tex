...
The tree, $T_1$, shown in Figure \ref{fig:t1} was built via the GN algorithm from the mailing list \emph{prior} to the introduction of the  ToSBack bot corresponds to a nested hierarchy of communities within the ToS;DR mailing list at that time. In particular the vertex labeled 1 represents the entire mailing list and every interaction between contributors, whereas the vertices 2 and 17 represent two sub-communities that we will call C2 and C17 respectively. Since, in addition to vertex 1, 2 is adjacent to 3 and 4 there also exist sub-communities of C2 that we denote C3 and C4. One can easily deduce from the GN algorithm that leaf vertices (vertices incident to exactly one edge) of $T_1$ correspond to a single contributor, thus C3 is a community of precisely 1 person. On the other hand C4 indicates a much larger community – (C2 without one contributor).
