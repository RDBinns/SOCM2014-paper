...
\[\beta(e_{ij}) = \sum_{ u\neq v \neq t \in V(G)} \frac{\sigma_{st}(e_{ij})}{\sigma_{st}}\]
where $\sigma_{st}$ is the number of shortest paths from vertex $s$ to $t$ and $\sigma_{st}(e_{ij})$ is the number of those shortest paths that pass through $e_{ij}$.  
...
\includegraphics[width=7.5cm]{graph1}\caption[width=7]{ $G_1$, network graph corresponding to the mailing list prior to the ToSBack bot contributions, with the longest branch colored in red}
...
\includegraphics[width=7.5cm]{graph2}\caption{ $G_2$, network graph corresponding to the mailing list after the ToSBack bot contributions began}
